...
import com.naztech.lms.model.YearlyStatusStrategicDashboard;
...
public class LmsDashboardService extends AbstractService<LmsDashboard> {
	...
	public Message<?> serviceSingle(Message msg) throws Exception {
		...
		try {
			...
			else if(action.equals(ActionType.LOAD_STRATEGIC_DASHBOARD.toString())) {
				msgResponse = MessageBuilder.withPayload(handle_Strategic_Dashboard(msg, action)).copyHeadersIfAbsent(msgHeader).build();				
			}
			else {
	...
	private List<YearlyStatusStrategicDashboard> handle_Strategic_Dashboard(Message<List<LmsDashboard>> msg, String action) throws Exception {
		List<LmsDashboard> list = msg.getPayload();
		if (list == null || list.size() == 0) {
			throw new Exception("Empty Request.");
		}
		return doHandleStrategicDashboard(list.get(0), action);
	}
	
	private List<YearlyStatusStrategicDashboard> doHandleStrategicDashboard(LmsDashboard lmsDash, String action) throws Exception {
		try {			
			
			JdbcResult jdbcResult = exectuteTrackerDept(lmsDash, action);
			return JdbcUtils.mapRows(YearlyStatusStrategicDashboard.class, YearlyStatusStrategicDashboard.getRs2BeanMap(),
			        jdbcResult.getRsTypeMap(RSType.RS_TYPE_STRATEGIC_FIELD_OFFICER.toString()));
