...

{
    ...
    IF (@tx_action_name IN (_ACTION_NEW, _ACTION_UPDATE,_ACTION_DELETE, 'FO_CREATE', 'FO_UPDATE', 'SO_CREATE' , 'FO_DELETE', 'SO_UPDATE', 'FO_SUBMIT', 'SO_RECOMMEND', 'SO_RE_RECOMMEND', 'MIS_UPDATE'
    , 'MIS_ALLOCATE', 'MIS_RE_ALLOCATE', 'FO_DELETE', 'SO_DELETE', 'CA_UPDATE', 'CA_RECOMMEND', 'CA_RE_RECOMMEND') )
    {   
    ...
    IF ( @tx_action_name = 'SELECT_LOAN_FOR_GRID' )
    {
        ...
        */
        ...
        BEGIN
           ...
           OR tx_state_name IN ('RM_APPROVED', 'UH_APPROVED')
    ...
    IF(@tx_action_name = 'SEARCH_LOAN_GROUP_DATA')
    {
        SELECT DISTINCT id_loan_key AS id_to_keep_distinct
            , L.*
            ,C.tx_customer_id 
        ...
        FROM #TEMP_GROUP_LOAN_FOR_GRID LG
        JOIN T_LEGAL_ENTITY LE ON LE.id_legal_entity_key = LG.id_legal_entity_key
        JOIN T_USER USR ON USR.id_user_key = LG.id_creator_key
        ORDER BY LG.tx_loan_group_id DESC
        ...
        */
        ...
        DECLARE @l_id_group_filter_role_key int


        SELECT  @l_id_group_filter_role_key =  r.id_role_key
        FROM    V_GROUP_ROLE        R
        JOIN    V_USER_GROUP        GRP     ON GRP.id_group_key = R.id_group_key
        JOIN    T_USER              USR     ON USR.id_user_key  = GRP.id_user_key
        WHERE   USR.id_user_key     = ISNULL(@id_user_mod_key , USR.id_user_key)
        ...
        IF(@l_id_group_filter_role_key IS NOT NULL )
        {
           SELECT tx_rs_type = 'RS_TYPE_LOAN_GROUP_FOR_GRID', *, U.tx_login_name AS tx_loan_group_creator
           , L.tx_loan_group_id as tx_folder_name
           FROM #TEMP_LOAN_FOR_GRID_4 L
           JOIN T_USER U ON U.id_user_key = L.id_loan_group_creator_key
           where (int_recommend_group_key = @l_id_group_filter_role_key 
           OR tx_state_name IN ('RM_APPROVED', 'UH_APPROVED'))
           AND tx_loan_group_id IS NOT NULL
        }   
        ELSE
        {
           SELECT tx_rs_type = 'RS_TYPE_LOAN_GROUP_FOR_GRID', *
           , tx_loan_group_id as tx_folder_name
           FROM #TEMP_LOAN_FOR_GRID_4
        }
        IF(@tx_action_name = 'SEARCH_LOAN_GROUP_DATA' )
        {
            SELECT tx_rs_type = 'RS_TYPE_LOAN_SEARCH_FOR_GRID', *
            , tx_loan_group_id as tx_folder_name
            FROM #TEMP_LOAN_FOR_GRID_4
        }
    }
    ...
    IF ( @tx_action_name in( 'STATE_TRANSITION', 'RM_C_APPROVE', 'RM_APPROVE'
        ...
        , 'BM_RETURN', 'BOM_RECOMMEND', 'BOM_RETURN', 'PPC_RECOMMEND', 'PPC_RETURN', 'CEO_RETURN', 'APPROVED_RETURN', 'CAD_RETURN') )
    {
    ...
    'SO_RE_RECOMMEND', 'MIS_UPDATE', 'MIS_ALLOCATE', 'MIS_RE_ALLOCATE', 'CA_UPDATE', 'CA_RECOMMEND', 'CA_RE_RECOMMEND') )
    {
    ...
    IF(@tx_action_name = 'SELECT_ALL_LOAN_GROUP_DATA')
    {
        ...
        SELECT tx_rs_type = 'RS_TYPE_LOAN_GROUP_FOR_GRID'
            ...
            , LG.tx_loan_group_id as tx_folder_name
        FROM        #TEMP_LOAN_FOR_GROUP_GRID LG
        JOIN        T_LEGAL_ENTITY LE ON LE.id_legal_entity_key = LG.id_legal_entity_key
        JOIN        T_USER USR ON USR.id_user_key = LG.id_creator_key
        JOIN        T_USER U ON U.id_user_key = LG.id_loan_group_creator_key
        WHERE       tx_loan_group_id IS NOT NULL
        ORDER BY    LG.tx_loan_group_id DESC
    }
    IF(@tx_action_name = 'SELECT_FOR_ADD_TO_LOAN_GROUP')
    {
        SELECT DISTINCT id_loan_key AS id_to_keep_distinct
            ...
            , T.tx_fsm_type_name
            , M.int_permission
        INTO #TEMP_LOAN_FOR_ADD_TO_LOAN_GROUP_GRID
        FROM T_LOAN L
        JOIN T_FSM_STATE S ON L.id_state_key = S.id_fsm_state_key
        JOIN T_FSM_TYPE T ON S.id_fsm_type_key = T.id_fsm_type_key AND T.tx_fsm_type_name = 'LOAN'
        JOIN T_ROLE_STATE_MAP M ON M.id_fsm_state_key = S.id_fsm_state_key and M.is_active = 1
        ...
        SELECT tx_rs_type = 'RS_TYPE_LOAN_GROUP_FOR_GRID' 
            ...
            , LE.tx_legal_entity_name
            , USR.tx_login_name  AS tx_creator_name
        FROM        #TEMP_LOAN_FOR_ADD_TO_LOAN_GROUP_GRID LG
        JOIN        T_LEGAL_ENTITY LE ON LE.id_legal_entity_key = LG.id_legal_entity_key
        JOIN        T_USER USR ON USR.id_user_key = LG.id_creator_key
        WHERE       tx_loan_group_id  IS NULL
        AND         tx_state_name IN ('RM_APPROVED', 'UH_APPROVED')
    }
