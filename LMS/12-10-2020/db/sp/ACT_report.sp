...
_CREATE_PROC (_SP_NAME)
    ...
    , @dec_business_recomm}ed_amnt    DECIMAL(20,2)          = NULL
...

{
    ...
    IF(@tx_action_name ='LMS_MEMO_REPORT_PDF')
    {
        ...
        SELECT tx_rs_type = 'RS_TYPE_TOTAL_AMOUNT_FILE'
            ...
            , SUM(T.dec_applied_loan_amount) AS TotalAmount        
        FROM T_LOAN T
    ...
    IF (@tx_action_name = 'LMS_LOAN_EXCEL_REPORT' OR @tx_action_name = 'PPC_EXCEL_REPORT')
    {
        ...
        IF (@tx_action_name = 'LMS_LOAN_EXCEL_REPORT')
        {
            ...
            , (SELECT TOP 1 AUD.dtt_mod from T_LOAN_AUDIT AUD JOIN T_FSM_STATE ST ON ST.id_fsm_state_key=AUD.id_state_key WHERE AUD.id_loan_key = @id_loan_key AND ST.tx_state_name='MIS_RECEIVED' ORDER BY AUD.dtt_mod DESC) AS dtt_crm_received_date
